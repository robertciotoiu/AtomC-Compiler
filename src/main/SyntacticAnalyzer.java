...
public class SyntacticAnalyzer {
	...
	public boolean declStruct(int i) throws AnalyzerException {
		...
		while (true) {
			...
			case 2:
				...
				// } else {
				// throw new AnalyzerException(
				// "Expected OpeningCurlyBrace" + tokens.get(i).getBegin() + " and at token: " +
				// i);
				// }
				break;
			case 3:
		...
		while (true) {
			...
			case 2:
				if (exprAssign(i)) {
					...
					return true;
					// throw new AnalyzerException(
					// "Expected exprAssign at: " + tokens.get(i).getBegin() + " and at token: " +
					// i);
				}
			case 3:
		...
		while (true) {
			...
			case 0:
				if (tokens.get(i).getTokenType().equals(TokenType.OpenBrace)) {
					state = 1;
					i = consume();
				} else if (!exprUnary(i)) {//maybe negated
					return true;
				} else {
					return false;
	...
	public boolean exprUnary(int i) throws AnalyzerException {
		...
		while (true) {
			...
			case 0:
				...
				break;
			case 1:
				if (!exprPostfix(i))// cred ca negat
				{
					return true;
				} else
					return false;
	...
	public boolean exprMul(int i) throws AnalyzerException {
		...
		if (exprMul1(i)|| imp)
			return true;
		//aici trebuie pus i = this.i si peste tot in rest #problema
		return false;
	}
